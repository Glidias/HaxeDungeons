...
#end
{
	...
	public static function getControlUnderMouse():Control
	{
		if (Control.controlUnderMouse == null) {
			var entity:Entity = HXP.scene.collidePoint("control", Mouse.mouseX + HXP.camera.x, Mouse.mouseY + HXP.camera.y);
	 ...
	 * @param	c
	 */
	public function removeControl(child:Control):Bool {
	...
	public function enableChildrenRecursively():Void
	{
		this.enabled = true;
		for (c in children) {
			c.enableChildrenRecursively();
		}
	}
	...
	override  function get_localX():Float
	{
		return (_container == null) ? x : _localX;
	}
	override function set_localX(value:Float):Float
	{
		_localX = value;
	...
	override function get_localY():Float
	{
		return (_container == null) ? y : _localY;
	}
	override function set_localY(value:Float):Float
	{
		_localY = value;
	...
	var myEnabled(get, set):Bool;
	function get_myEnabled():Bool
	{
		return _enabled;
	}
	function set_myEnabled(value:Bool):Bool
	{
		//active = value;
		//super.enabled = value;
