...
class Label extends Control
{
	 ...
	 * Those labels are always accessible using "myComponent.label" and you can change specific Labels apperence any time.
	 */
	public static var defaultSize:Null<Int> = 8;
	/**
	...
	public function new(text:String = "", x:Float = 0, y:Float = 0, width:Int = 0, height:Int = 0, ?align:FormatAlign, ?valign:VerticalAlign)
	{
		...
		_textBuffer = new BitmapData(width, height, true);
		graphic = new Image(new Texture(_textBuffer));
	...
	{
		if (toBeRedraw) {
			_textBuffer.fillRect(_renderRect, 0);
	...
	{
		_textBuffer = new BitmapData(width + _bitmapBufferMargin * 2, height + _bitmapBufferMargin * 2, true);
		graphic = new Image(new Texture(_textBuffer));
	}
	...
	override function set_myEnabled(value:Bool):Bool
	{
		if (value) {
		...
		updateBuffer();
		return super.set_myEnabled(value);
	}
